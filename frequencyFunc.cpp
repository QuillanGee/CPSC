...
//calculates the frequency with an octave and a half tone
double calcFrequency(int v, int k){
  double refFrequency = 16.35;
  double frequency = refFrequency * pow(2,(v)+(k/12.0));
  return frequency;
}

//calculates the wavelength of the note with the octave and the half tone
double calcWavelength(int v, int k){
  double wavelength;
  int c = 34500;
  wavelength = c/calcFrequency(v,k);
  return wavelength;
}

//prints the frequency and wavelength respective to each note
int main() {
  string a_strNotes[] = {
