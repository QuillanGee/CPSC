...
/// @author outstanding.qgee@chapman.edu
...
bool Game::isCloseToTarget(int iRow, int iColumn)
{
	...
	// the target boat.
	// /\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\ //
	if ((abs(iRow - this->m_iRowBoat) <= 1 && abs(iRow - this->m_iRowBoat) >=0) || (abs(iColumn - this->m_iColumnBoat) <= 1 && abs(iColumn - this->m_iColumnBoat) >=0 ))
	{
		bClose = true;
	}
